...
export default function ContentEditor(props) {
  ...
  const [title, setTitle] = React.useState(
    initialPost?.title || "Bài viết mới"
  );
  const [description, setDescription] = React.useState(
    initialPost?.description || "Bài viết này về..."
  );
  const [imagePath, setImagePath] = React.useState(
    initialPost?.thumbnail || null
  );
  const [category, setCategory] = React.useState(categories[0]);
  const [content, setContent] = React.useState(
    initialPost?.content || "<h1>Nội dung bài viết ở đây\n<h1><br/>"
  );
          ...
          <Button
            ...
            onClick={async () => {
              ...
              const result = await creatorApi.editPost({
                ...
                thumbnail: imagePath
                  ? imagePath
                  : "https://vieshare-stg.vi-vu.vn/default.jpg",
              });
              ...
              if (result.data) {
                ...
                alert(
                  "Đã có lỗi xảy ra khi chúng tôi lưu bài viết, chi tiết: " +
                    result.message
                );
              }
              // TODO
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (result.data.id) {
                  ...
                  alert(
                    "Đã lưu bài viết vào bản nháp. Hãy phát hành bài viết khi bạn đã chắc chắn rằng nó đã hoàn thiện nhé!"
                  );
                } else {
                  if (result.statusCode === "INVALID_CREDENTIALS") {
                    return;
                  ...
                  alert(
                    "Đã có lỗi xảy ra khi chúng tôi thử lưu bài viết, chi tiết: " +
                      result.message
                  );
                }
                // TODO

...
export default function SecurityTab({ profile, yup }) {
  ...
  const handleSave = async () => {
    if (
      (!profile.isDefaultPassword && formik.errors.oldPassword) ||
    ...
    profileAPI
      ...
      .catch((err) => {
        updatePassworFailed(
          err.response.data.data?.toString() || err.response.data.message
        );
      });
  ...
  return (
    <>
      <Card
        sx={{ maxWidth: "100%", border: 1, mt: 1, borderColor: "grey.500" }}
      >
        <HeadersTab
        ...
        <Card className={styles["modal-card"]}>
          <form onSubmit={formik.handleSubmit} noValidate>
            ...
            <CardActions sx={{ justifyContent: "flex-end" }}>
              ...
              <Button size="small" type="submit" onClick={handleSave}>
                Lưu
              </Button>
            </CardActions>
          </form>
